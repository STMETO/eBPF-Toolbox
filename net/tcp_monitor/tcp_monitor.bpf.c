...
int BPF_PROG(trace_tcp_finish_connect, struct sock *sk, struct sk_buff *skb)
{
	...
	if (stats) {
		...
		if (latency_ns > stats->hs_max_ns) {
			...
			stats->hs_max_sport = event->sport;
			stats->hs_max_dport = event->dport;
			stats->hs_max_af = event->af;
			stats->hs_max_saddr = event->saddr_v4;
			stats->hs_max_daddr = event->daddr_v4;
			__builtin_memcpy(stats->hs_max_saddr_v6, event->saddr_v6,
					 sizeof(stats->hs_max_saddr_v6));
			__builtin_memcpy(stats->hs_max_daddr_v6, event->daddr_v6,
