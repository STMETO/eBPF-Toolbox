...
#include "Schedule_Delay.h"
#include "perf/Schedule_Delay.skel.h"
...
int schedule_delay_run(int poll_timeout_ms, bool enable)
{
	struct Schedule_Delay_bpf *skel = NULL;
	struct Schedule_Delay_ctrl ctrl = {.enable = enable};
	const int key = 0;
	int err = 0;

	skel = Schedule_Delay_bpf__open_and_load();
	if (!skel) {
	...
	err = Schedule_Delay_bpf__attach(skel);
	if (err) {
...
cleanup:
	Schedule_Delay_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
