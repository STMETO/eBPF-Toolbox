...
{
	struct SysStat_bpf *skel = NULL; struct ring_buffer *rb = NULL;
	struct SysStat_ctrl ctrl = { .enable = enable }; const int key = 0; int err = 0;
	skel = SysStat_bpf__open_and_load();
...
cleanup: ring_buffer__free(rb); SysStat_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
