...
#include "MsgQueue_Delay.h"
#include "perf/MsgQueue_Delay.skel.h"
...
{
	// BPF 程序骨架（自动生成的结构体）
	struct MsgQueue_Delay_bpf *skel = NULL;
	// 环形缓冲区：内核 → 用户态 传递事件
	...
	skel = MsgQueue_Delay_bpf__open_and_load();
	if (!skel) {
	...
	// 把 BPF 程序挂载到内核钩子点
	err = MsgQueue_Delay_bpf__attach(skel);
	if (err) {
...
cleanup:
	...
	MsgQueue_Delay_bpf__destroy(skel);       // 卸载 BPF 程序
