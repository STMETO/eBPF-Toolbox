#include <errno.h>
#include <inttypes.h>
#include <signal.h>
#include <stdio.h>
#include <string.h>
#include <unistd.h>
#include <bpf/libbpf.h>
#include "common/cli.h"
#include "common/types.h"
#include "common/logger.h"
#include "write.h"
#include "fs/write/skel.h"

static struct write_bpf *g_skel = NULL;

static void print_stats(void)
{
	if (!g_skel) return;
	struct Write_stats s = {}; int key = 0;
	if (bpf_map__lookup_elem(g_skel->maps.stats_map, &key, sizeof(key),
				 &s, sizeof(s), 0) || !s.count) return;
	fprintf(stderr, "\n");
	printf(C_CYAN C_BOLD "══════ Write 统计 ══════\n" C_RESET);
	printf("  采样: %" PRIu64 " 次\n", s.count);
	printf(C_CYAN C_BOLD "══════════════════════\n" C_RESET);
}
static void sig_handler(int sig) { (void)sig; print_stats(); _exit(0); }

static int handle_event(void *ctx, void *data, size_t data_sz)
{
	const struct Write_event *e = data; (void)ctx; (void)data_sz;
	LOG("PID=%-6d FD=%-4d REQ=%-8lld ACTUAL=%-8lld %-16s %s\n",
	    e->pid, e->fd, (long long)e->count, (long long)e->real_count, e->comm,
	    e->path_name_[0] ? e->path_name_ : "(unknown)");
	return 0;
}

int write_run(int poll_timeout_ms, bool enable,
	      bpf_s32_t target_pid, bpf_u64_t min_delay_ns)
{
	struct write_bpf *skel = NULL; struct ring_buffer *rb = NULL;
	const int key = 0; int err = 0;
	skel = write_bpf__open_and_load();
	if (!skel) { fprintf(stderr, "打开BPF程序失败\n"); return 1; }
	g_skel = skel;
	struct Write_ctrl ctrl = {.enable = enable, .min_delay_ns = min_delay_ns,
				  .target_pid = target_pid};
	err = bpf_map__update_elem(skel->maps.ctrl_map, &key, sizeof(key),
				   &ctrl, sizeof(ctrl), BPF_ANY);
	if (err < 0) { fprintf(stderr, "设置控制开关失败\n"); goto cleanup; }
	signal(SIGINT, sig_handler); signal(SIGTERM, sig_handler);
	rb = ring_buffer__new(bpf_map__fd(skel->maps.rb), handle_event, NULL, NULL);
	if (!rb) { err = -ENOMEM; fprintf(stderr, "创建RingBuffer失败\n"); goto cleanup; }
	err = write_bpf__attach(skel);
	if (err) { fprintf(stderr, "挂载BPF程序失败\n"); goto cleanup; }
	log_banner("文件 Write 监控", enable);
	if (target_pid) LOG("过滤 PID=%d\n", target_pid);
	LOG_HDR("%-7s %-5s %-9s %-9s %-16s %s", "PID", "FD", "REQ", "ACTUAL", "COMM", "PATH");
	LOG_SEP();
	while (!app_should_exit()) { err = ring_buffer__poll(rb, poll_timeout_ms); if (err == -EINTR) { err = 0; break; } if (err < 0) break; }
	print_stats();
cleanup: g_skel = NULL; ring_buffer__free(rb); write_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
