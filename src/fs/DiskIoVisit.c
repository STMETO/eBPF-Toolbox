...
{
	struct DiskIoVisit_bpf *skel = NULL;
	struct ring_buffer *rb = NULL;
	struct DiskIoVisit_ctrl ctrl = { .enable = enable };
	const int key = 0;
	int err = 0;

	skel = DiskIoVisit_bpf__open_and_load();
	if (!skel) {
	...
	err = DiskIoVisit_bpf__attach(skel);
	if (err) {
