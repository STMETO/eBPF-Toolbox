...
#define NR_VM_ZONE_STAT_ITEMS 5		// vm zone 内存统计项数量，限定 vm_stat 数组长度
#define NR_FREE_PAGES 0				// vm_stat 数组下标0：空闲物理页计数项
 ...
 * @field vm_stat 回收触发瞬间整机内存统计快照数组，共NR_VM_ZONE_STAT_ITEMS项
 */
struct val_t {
	bpf_u64_t id;
	bpf_u64_t ts;
	char name[TASK_COMM_LEN];
	bpf_u64_t vm_stat[NR_VM_ZONE_STAT_ITEMS];	// 每一项都对应内存快照中的一个数值
};
