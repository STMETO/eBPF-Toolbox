...
#include "MutexLock_Delay.h"
#include "perf/MutexLock_Delay.skel.h"
...
int mutexlock_delay_run(int poll_timeout_ms, bool enable)
{
	struct MutexLock_Delay_bpf *skel = NULL;
	struct ring_buffer *rb = NULL;
	struct MutexLock_Delay_ctrl ctrl = {.enable = enable};
	const int key = 0;
	int err = 0;

	skel = MutexLock_Delay_bpf__open_and_load();
	if (!skel) {
	...
	err = MutexLock_Delay_bpf__attach(skel);
	if (err) {
...
cleanup:
	...
	MutexLock_Delay_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
