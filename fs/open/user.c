...
{
	if (!g_skel) return; struct Open_stats s = {}; int key = 0;
...
{
	const struct Open_event *e = data; (void)ctx; (void)data_sz;
...
{
	struct open_bpf *skel = NULL; struct ring_buffer *rb = NULL;
	const int key = 0; int err = 0;
	skel = open_bpf__open_and_load();
	if (!skel) { fprintf(stderr, "打开BPF程序失败\n"); return 1; }
	g_skel = skel;
	struct Open_ctrl ctrl = {.enable = enable, .min_delay_ns = min_delay_ns, .target_pid = target_pid};
	...
	print_stats();
cleanup: g_skel = NULL; ring_buffer__free(rb); open_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
