...
#include "Preempt_Delay.h"
#include "perf/Preempt_Delay.skel.h"
...
{
	struct Preempt_Delay_bpf *skel = NULL;
	struct ring_buffer *rb = NULL;
	struct Preempt_Delay_ctrl ctrl = {.enable = enable};
	const int key = 0;
	int err = 0;

	skel = Preempt_Delay_bpf__open_and_load();
	if (!skel) {
	...
	err = Preempt_Delay_bpf__attach(skel);
	if (err) {
...
cleanup:
	...
	Preempt_Delay_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
